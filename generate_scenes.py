#!/usr/bin/env python3
"""Generate additional scene backgrounds: backroom and dawn."""
...
SCENES = [
    {
        "name": "bg_backroom",
        "out": os.path.join(IMG_DIR, "bg_backroom.png"),
        "w": 512, "h": 896,
        "prompt": (
            "anime style, retro anime, japanese convenience store back storage room, "
            "dim single overhead light, cardboard boxes stacked, old metal filing cabinet, "
            "dusty shelves with old ledgers and papers, narrow space, warm amber light, "
            "visual novel background art, detailed scenery, moody atmosphere, no people"
        ),
    },
    {
        "name": "bg_dawn",
        "out": os.path.join(IMG_DIR, "bg_dawn.png"),
        "w": 512, "h": 896,
        "prompt": (
            "anime style, retro anime, japanese convenience store interior at dawn, 5am, "
            "pale blue-pink morning light coming through large glass doors, "
            "wet street outside reflecting early sky, shelves slightly dim, "
            "peaceful quiet morning atmosphere, visual novel background art, "
            "soft cool dawn color palette, no people, no characters"
        ),
    },
]

def build_workflow(prompt, seed, prefix, w, h):
    return {
...
for scene in SCENES:
    if os.path.exists(scene["out"]):
        print(f"[{scene['name']}] already exists, skipping")
        continue
    seed = random.randint(0, 2**31)
    print(f"\n[{scene['name']}] seed={seed}…")
    wf = build_workflow(scene["prompt"], seed, f"cs_{scene['name']}", scene["w"], scene["h"])
            ...
            if entry.get("outputs"):
                ...
                for node_out in entry["outputs"].values():
                    for i in node_out.get("images", []):
                        ...
                        urllib.request.urlretrieve(f"{BASE}/view?{params}", scene["out"])
                        print(f"  Done in {elapsed}s → {os.path.basename(scene['out'])}")
                break
